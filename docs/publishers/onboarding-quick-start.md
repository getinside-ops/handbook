---
title: Démarrage Rapide — Distributeurs
---

# Démarrage Rapide — Distributeurs

Vous venez de rejoindre getinside. En 3 étapes (10 minutes), votre profil est prêt à recevoir vos premières campagnes.

::: tip À qui s'adresse ce guide ?
**Ce guide est pour vous si :** vous avez reçu l'email d'invitation getinside et souhaitez démarrer rapidement.
:::

---

## Étape 1 — Activez votre accès (2 min)

1. Ouvrez l'**email d'invitation** reçu de getinside
2. Cliquez sur le lien d'activation
3. Définissez votre mot de passe
4. Connectez-vous sur [app.getinside.media](https://app.getinside.media/)

::: info
Votre compte a été créé par l'équipe getinside lors de votre démo. Si vous n'avez pas reçu l'email, vérifiez vos spams ou contactez votre gestionnaire.
:::

---

## Étape 2 — Configurez votre profil logistique (5 min)

Avant de recevoir des campagnes, l'algorithme a besoin de deux informations :

**Espace :** Paramètres → Logistique

| Champ | Quoi renseigner |
|-------|----------------|
| **Adresse entrepôt** | L'adresse où les supports physiques (cartons, flyers) seront livrés |
| **Lead Time** | Le délai minimum entre réception et début de distribution (ex: 3 jours) |

::: info
Ces données permettent à la plateforme de vous proposer uniquement des campagnes que vous pouvez honorer selon vos contraintes réelles.
:::

---

## Étape 3 — Créez votre première audience (3 min)

Votre audience, c'est ce que vous vendez aux annonceurs. Plus elle est précise, plus vous recevez de campagnes pertinentes.

**Espace :** Monétisation → Créer une audience

Renseignez au minimum :
- **Nom de l'audience** : ex. "Clients e-commerce mode femme 25-45 ans"
- **Panier moyen** de vos clients
- **Catégorie** de vos produits
- **Volume hebdomadaire** de colis (pour l'Insertion Colis)

::: tip
Une fiche audience complète et précise est le principal facteur d'attractivité auprès des annonceurs. Prenez le temps de bien la remplir.
:::

---

## Vous êtes prêt ✅

Un **appel de validation** avec l'équipe getinside confirme votre mise en ligne sur la marketplace. Dès lors, vous recevrez des **propositions de campagnes** directement sur la plateforme.

**Règle d'or à retenir :** Vous avez **48h ouvrées** pour répondre à chaque proposition. Au-delà, elle expire et peut impacter votre [Score Distributeur](/publishers/score-distributor).

---

**Prochaines étapes :**
- [Processus complet d'onboarding](/publishers/onboarding-process) — le fonctionnement détaillé
- [Score Distributeur](/publishers/score-distributor) — comment maximiser votre visibilité
- [Paiements & Wallet](/publishers/payouts) — comprendre quand et comment vous serez payé
